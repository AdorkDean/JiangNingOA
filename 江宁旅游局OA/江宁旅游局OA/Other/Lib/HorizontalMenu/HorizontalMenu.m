...
- (void)creatTopToolBar:(NSArray *)titles
{
    ...
    for (int i = 0; i < titles.count; i++) {
        ...
        [button setTitleColor:XHColor(32, 40, 47) forState:UIControlStateNormal];
        [button setTitleColor:XHColor(82, 143, 229) forState:UIControlStateSelected];
        [button addTarget:self action:@selector(btnAction:) forControlEvents:UIControlEventTouchUpInside];
        UIImage *btnImg = [UIImage imageNamed:@"导航箭头"];
        [button setImage:btnImg forState:UIControlStateSelected];
         [button setImageEdgeInsets:UIEdgeInsetsMake(0, 77, 0, 20) ];
        button.tag = i;
//        [button setBackgroundColor:XHColor(241, 242, 244)];
   ...
   // CGSize size = [Utility sizeWithText:_tmpBtn.titleLabel.text font:_tmpBtn.titleLabel.font maxSize:CGSizeMake(MAXFLOAT, MAXFLOAT)];
    ...
    //选中线
    CGFloat selectLineW = SCREEN_WIDTH / titles.count;
    ...
    _selectLine.backgroundColor = XHColor(82, 143, 229);
